...
uint32_t strLength(const uint8_t *uintString)
{
	int i = 0;
	for(i = 0; uintString[i] != 0; i++)
	{

	}
	return i;
}
CMD_HANDLER_FUNC(myi2c2Handler)
{
	static bool slaveInit = false;
	static bool masterInit = false;
	static const uint8_t slaveDeviceAddress = 0x80;
	static uint8_t masterReadBuffer[256] = {'M', 'A', 'S', 'T','E','R'};
	static uint8_t slaveBuffer[256] = {'B','U','F','F','E','R'};

	I2C2& i2cInstance = I2C2::getInstance();
	if(cmdParams.beginsWith("mm"))
	{
		if(masterInit == true)
		{
	...
	else if(cmdParams.beginsWith("ss"))
	{
		if(slaveInit == true)
		{
			printf("I2C2 Slave has already been initialized.\n");
			return true;
		}
		else if(slaveInit==false)
		{
			slaveInit = true;
			i2cInstance.i2c2SlaveInit(slaveDeviceAddress,slaveBuffer,sizeof(slaveBuffer));
			printf("I2C2 initialized as a Slave Device!!\n");
			return true;
		}
	}
	else if(cmdParams.beginsWith("-d"))
	{
		if(masterInit == true)
		{
			if(i2cInstance.checkDeviceResponse(slaveDeviceAddress))
				printf("Slave has been discovered\n");
			else
				printf("Slave not discovered\n");

			return true;
		}
		else
		{
			printf("Slave cannot discover itself\n");
			return true;
		}
	}
	else if(cmdParams.beginsWith("-w"))
	{
		if(masterInit == false)
		{
