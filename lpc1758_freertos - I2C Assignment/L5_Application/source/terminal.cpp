...
bool terminalTask::taskEntry()
{
    ...
    cp.addHandler(myi2c2Handler,   "i",   "'i2c read 0x01 0x02 <count>' : Reads <count> registers of device 0x01 starting from 0x02\n"
                                           "'i2c write 0x01 0x02 0x03'   : Writes 0x03 to device 0x01, reg 0x02\n"
                                           "'i2c discover' : Discovers all I2C devices on the BUS");

    cp.addHandler(ledHandler,     "led",    "Turn on the led.  'Turn off the led");
