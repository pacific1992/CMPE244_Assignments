...
bool I2C_Base::i2cSlaveBaseInit(uint8_t slaveAddress, uint8_t* buffer, uint16_t sizeOfBuffer)
{
	//power i2c2
	switch(mIRQ)
	{
	...
	slaveBufferArray = buffer;
	slaveBufferSize = sizeOfBuffer;
	slaveDataCounter = 0;
	bufferIndex = 0;


	mpI2CRegs->I2CONSET = 0x4C;
...
I2C_Base::mStateMachineStatus_t I2C_Base::i2cStateMachine()
{
	...
	{
		//States I added
		slaveAddressWriteReceivedAckSent = 0x60,
		dataReceivedAckSent = 0x80,
		slaveRepeatStartOrStopReceived = 0xA0,
		slaveAddressReadReceivedAckSent = 0xA8,
		slaveTransmitMasterReadAckReceived = 0xB8,
		slaveTransmitMasterNackReceived = 0xC0,
	}rxStates_t;
...
#define setStop()           clearSTARTFlag();                           \
	...
	case arbitrationLost:
		...
		//My switch cases
	case slaveAddressWriteReceivedAckSent: //0x60
		slaveDataCounter = 0;
		setAckFlag();
		clearSIFlag();
		break;

	case dataReceivedAckSent: //0x80
		if(slaveDataCounter == 0) //first byte received
		{
			bufferIndex = mpI2CRegs->I2DAT;
			slaveDataCounter++;
		}

		else
		{
			slaveBufferArray[bufferIndex]=mpI2CRegs->I2DAT;
		//	u0_dbg_printf("Buffer array-->%c\n",slaveBufferArray[bufferIndex]);
			bufferIndex++;
			slaveDataCounter++;
		}
		setAckFlag();
	...
	case slaveRepeatStartOrStopReceived: //0xA0
		setAckFlag();
		clearSIFlag();
		break;

	case slaveAddressReadReceivedAckSent: //0xA8
	//	slaveBufferArray[bufferIndex]=LS.getPercentValue();
	//	printf
		mpI2CRegs->I2DAT = slaveBufferArray[bufferIndex];
		setAckFlag();
		clearSIFlag();
		bufferIndex++;
		break;

	case slaveTransmitMasterReadAckReceived: //0xB8
	//	slaveBufferArray[bufferIndex]=LS.getPercentValue();
		mpI2CRegs->I2DAT =slaveBufferArray[bufferIndex];
		bufferIndex++;
		setAckFlag();
	...
	case slaveTransmitMasterNackReceived: //0xC0
		setAckFlag();
		clearSIFlag();
		break;
